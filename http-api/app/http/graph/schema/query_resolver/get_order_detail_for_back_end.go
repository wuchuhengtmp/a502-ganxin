...
func (*QueryResolver) GetOrderDetailForBackEnd(ctx context.Context, input graphModel.GetOrderDetailForBackEndInput) (*orders.GetOrderDetailForBackEndRes, error) {
	...
	summaryIn := model.DB.Debug().Model(&orderSpecificationItem).
	...
	var weightInfo struct {
		...
		SteelTotal int64   `json:"SteelTotal"`
	}
	if err := summaryIn.
		Select("sum(weight * total) as Weight, sum(total) as SteelTotal").
		Scan(&weightInfo).Error; err != nil {
		return nil, errors.ServerErr(ctx, err)
	}
	res.Weight, _ = strconv.ParseFloat(fmt.Sprintf("%.3f", weightInfo.Weight), 10)
	res.SteelTotal = weightInfo.SteelTotal
