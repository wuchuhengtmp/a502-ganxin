...
func (*QueryResolver)GetProjectDetail(ctx context.Context, input graphModel.GetProjectDetailInput) (*projects.GetProjectDetailRes, error) {
	...
	if input.OutOfRepositoryAt != nil {
		startTme, _ := helper.Str2Time(helper.Time2Str(*input.OutOfRepositoryAt))
		endTime := startTme.Add(time.Second).Format(time.RFC3339Nano)
		modelIns = modelIns.Where(fmt.Sprintf("%s.out_repository_at BETWEEN ? AND ?", orderSteelItem.TableName()), startTme.Format(time.RFC3339Nano), endTime)
	}
	// 入库时间
	if input.EnterRepositoryAt != nil {
		modelIns = modelIns.Where(fmt.Sprintf("%s.enter_repository_at = ?", orderSteelItem.TableName()), *input.EnterRepositoryAt)
	}
	//  入场时间
	if input.EnteredWorkshopAt != nil {
		modelIns = modelIns.Where(fmt.Sprintf("%s.enter_workshop_at = ?", orderSteelItem.TableName()), *input.EnteredWorkshopAt)
	}
	// 出场时间
	if input.OutOfWorkshopAt != nil {
		modelIns = modelIns.Where(fmt.Sprintf("%s.out_of_workshop_at = ?", orderSteelItem.TableName()), *input.OutOfWorkshopAt)
	}
	// 安装编码
